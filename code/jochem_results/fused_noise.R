...
for(j in 1:lnu){
  for(i in 1:L1){
    ...
    for(i1 in 1:lno){
      ...
      gpls1<-plsr(y~.,data=data.frame(ytr,x)[L,])
      gpls2<-plsr(y~.,data=data.frame(ytr,z)[L,])
      dat1=cbind(gpls1$scores[,1:2],gpls2$scores[,1:2])
      g1<-gam(y~.,data=data.frame(y=ytr[L],x=dat1))
  ...
  write.table(acc,"acc_noise_fused_pls.txt",quote=FALSE,row.names=FALSE,col.names=FALSE)
